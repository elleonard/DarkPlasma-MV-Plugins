...
/**
 * 2021/10/15 1.3.0 rollup構成へ移行
 *                  指定パーティの位置を取得するインターフェース追加
 ...
 *
 * @orderAfter DarkPlasma_ConcurrentParty
 *
 ...
 * DarkPlasma_ConcurrentParty version:1.3.0
 */
...
(() => {
  ...
  class Game_DevidedParties {
    ...
    savePartyPosition(index) {
      this.getParty(index).setPosition($gameMap.mapId(), $gamePlayer.x, $gamePlayer.y, $gamePlayer.direction());
    }
  ...
  function Game_Party_ConcurrentPartyMixIn(gameParty) {
    ...
    gameParty.setDevidedPartyPosition = function (partyIndex, mapId, x, y, direction) {
      const targetParty =
        partyIndex >= 0 ? this.devidedParties().getParty(partyIndex) : this.devidedParties().lastParty();
      if (targetParty) {
    ...
    gameParty.swapOrder = function (index1, index2) {
      if (this.isDevided()) {
        ...
        this.devidedParties().getCurrentParty().swapOrder(allMembers[index1].actorId(), allMembers[index2].actorId());
      } else {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    switch (command) {
      ...
      case 'moveParty':
        if ($gameParty.isDevided()) {
          $gameParty.setDevidedPartyPosition(
            Number(args[0]),
            Number(args[1]),
            Number(args[2]),
            Number(args[3]),
