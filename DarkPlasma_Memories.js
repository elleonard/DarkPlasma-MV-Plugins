// DarkPlasma_Memories 3.0.0
// Copyright (c) 2017 DarkPlasma
// This software is released under the MIT license.
// http://opensource.org/licenses/mit-license.php

/**
 * 2021/09/09 3.0.0 rollup構成に移行
 *                  シーン全開放プラグインコマンドを追加
 ...
 * 2020/04/30 1.3.1 合成画像でない場合にエラーになる不具合を修正
 *            1.3.0 Memories.json の合成画像に関する記法を拡張
 * 2020/04/16 1.2.0 DarkPlasma_ImageComposer.js に対応
 *            1.1.1 リファクタ
 * 2019/01/10 1.1.0 回想モードのウィンドウカラーを設定項目に追加
 * 2018/09/30 1.0.3 ブラウザでプレイするとシーン一覧が点滅する不具合の修正
 * 2017/11/04 1.0.2 未開放シーンを閲覧できる不具合の修正
 *            1.0.1 キャンセルキーで回想モードから抜けられるように
 ...
 * @text ウィンドウ色
 * @type struct<Tone>
 * @default {"red":"0", "green":"0", "blue":"100"}
 *
 * @help
 * version: 3.0.0
...
(() => {
  ...
  const settings = {
    ...
    })(pluginParameters.bgm || '{}'),
    adultIcon: Number(pluginParameters.adultIcon || 84),
    blankThumbnail: String(pluginParameters.blankThumbnail || ''),
    sandboxMap: Number(pluginParameters.sandboxMap || 1),
    terminateLabel: String(pluginParameters.terminateLabel || 'END_SCENE'),
    waitAfterMemory: Number(pluginParameters.waitAfterMemory || 500),
    windowTone: ((parameter) => {
      const parsed = JSON.parse(parameter);
      return {
    ...
    })(pluginParameters.windowTone || '{"red":"0", "green":"0", "blue":"100"}'),
  };
    ...
    static fromDataMemories() {
      ...
      return new MemoriesManager(
        $dataMemories.tags.map((tag) => {
          return new MemoryTag(tag.text, tag.symbol, tag.switch ? switches[tag.switch] : true);
        }),
        $dataMemories.scenes.map(
          (scene) =>
            new MemoryScene(
              scene.title,
              scene.thumbnail,
  ...
  class MemoryPicture {
    ...
    imageNameList(index) {
      ...
      const additionalImageNames = this._additionals[index].map(
        (imageName) => `${this._prefix}${imageName}${this._suffix}.png`
      );
      return [`<${this.getFileName(index)}.png>`].concat(
        this._base ? additionalImageNames : additionalImageNames.slice(1)
      );
    }
  }
  ...
  class Window_MemoryList extends Window_Selectable {
    ...
    enabledItemsCount() {
      return this._data
        ? this._data.filter(function (memory) {
    ...
    makeItemList() {
      ...
      if (this._progressWindow) {
        this._progressWindow.setProgressAndMax(this.enabledItemsCount(), this.maxItems());
      }
    }
    ...
    drawItem(index) {
      if (this._data) {
        ...
        const thumbnailFile = this.isEnabled(memory) ? memory.thumbnail : settings.blankThumbnail;
